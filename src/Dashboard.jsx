...
function Dashboard({ user, onLogout, onUpdateUser, onChangePassword, allUsers, cloud }) {
  ...
  useEffect(() => {
    ...
    onSnapshot(ref, (snap) => { const data = snap.exists() ? snap.data() : {}; setTasks(Array.isArray(data?.tasks) ? data.tasks : []); setGoals(Array.isArray(data?.goals) ? data.goals : []); setWishes(Array.isArray(data?.wishes) ? data.wishes : []) })
    
    const metaRef = doc(cloud.db, 'households', cloud.householdId, 'meta', 'messages')
    onSnapshot(metaRef, (snap) => { const data = snap.exists() ? snap.data() : {}; setMessages(Array.isArray(data?.messages) ? data.messages : []) })
    
    const appRef = doc(cloud.db, 'households', cloud.householdId, 'meta', 'apps')
    onSnapshot(appRef, (snap) => { const data = snap.exists() ? snap.data() : {}; setStudyApps(Array.isArray(data?.apps) ? data.apps : []) })
  ...
  const handleBulkAdd = async () => {
    ...
    for (const line of lines) {
      const [kidName, weekdayStr, subjectName, hour, min] = line.split('\t')
      if (!kidName || !weekdayStr || !subjectName) continue
      ...
      const startTime = `${hour.padStart(2, '0')}:${min.padStart(2, '0')}`
      addTask(subjectName, '#7c9cff', startTime, 50, 'class', 'Book', targetKid, format(selectedDate, 'yyyy-MM-dd'), { weekday })
    }
    setBulkInput(''); setShowClassManager(false); alert('수업 일괄 등록 완료!')
  }
  ...
  const sendSurpriseMessage = async () => {
    ...
    setNewMessage(''); setShowMessageManager(false); alert('사랑의 메시지를 전송했어요! 💌')
  }
  ...
  const submitPasswordChange = async () => {
    ...
    if (res.ok) { setCurrentPassword(''); setNextPassword(''); setTimeout(() => setShowSettings(false), 2000) }
  }
  ...
  const handleDragEnd = (event) => {
    ...
    if (dragData?.type === 'palette' && over.id.toString().startsWith('hour-')) {
      const hour = over.data.current.hour; const startTime = `${hour.toString().padStart(2, '0')}:00`
  ...
  const addTask = (name, color, startTime, duration, type, icon = 'Book', targetKidId = activeKidId, targetDate = format(selectedDate, 'yyyy-MM-dd'), extra = {}) => {
    ...
    const newTask = { id: Math.random().toString(36).substr(2, 9), name, color, startTime, expectedEndTime, duration, type, icon, completed: false, date: targetDate, ...(type === 'class' ? { weekday: extra.weekday ?? getDay(parseISO(targetDate)) } : {}), ...extra }
    ...
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={(e) => { const d = e.active.data.current; if(d?.type==='palette') setActiveDragItem({type:'palette', subject:d.subject}); else if(d?.type==='task') setActiveDragItem({type:'task', task:d.task}); }} onDragEnd={handleDragEnd}>
      <div className="dashboard-shell" style={{ maxWidth: '1200px', margin: '0 auto', padding: isMobile ? '8px' : '20px' }}>
        ...
        <header className="glass" style={{ padding: isMobile ? '12px 15px' : '20px 30px', borderRadius: 'var(--radius-lg)', marginBottom: '12px', background: 'white' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '10px' : '20px' }}>
              <div style={{ position: 'relative' }}>
                ...
                {todayMessage && <button onClick={() => setShowSurprise(true)} className="animate-bounce" style={{ position: 'absolute', top: '-10px', right: '-10px', background: '#ff4d6d', color: 'white', border: 'none', borderRadius: '50%', padding: '6px', cursor: 'pointer' }}><Gift size={isMobile ? 14 : 18} /></button>}
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '4px' : '10px' }}>
              ...
              <button onClick={() => { setShowGoals(!showGoals); setShowLogs(false); }} style={{ background: 'none', border: 'none', padding: '10px', color: 'var(--primary)' }}><Trophy size={isMobile ? 22 : 24}/></button>
          ...
          {isAdmin && (
            <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
              {kidsList.map(id => <button key={id} onClick={() => setActiveKidId(id)} style={{ flex: isMobile ? 1 : 'none', padding: isMobile ? '8px 12px' : '10px 20px', borderRadius: '10px', border: 'none', background: activeKidId === id ? 'var(--primary)' : 'rgba(0,0,0,0.05)', fontWeight: '800', color: activeKidId === id ? 'white' : 'var(--text-muted)', fontSize: '13px' }}>{allUsers[id]?.displayName || id}</button>)}
            </div>
          )}
        </header>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          <button onClick={() => { const todayStr = format(selectedDate, 'yyyy-MM-dd'); const tomorrowStr = format(addDays(selectedDate, 1), 'yyyy-MM-dd'); const todayTasks = tasks.filter(t => t.date === todayStr && t.type !== 'class'); if(todayTasks.length){ applyTaskChange(prev => [...prev, ...todayTasks.map(t => ({...t, id:Math.random().toString(36).substr(2,9), date:tomorrowStr, completed:false}))]); alert('내일로 복사 완료!'); } }} style={{ flex: 1, padding: '12px', borderRadius: '15px', border: 'none', background: 'white', fontWeight: '800', boxShadow: 'var(--shadow)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}><Copy size={16}/> 내일복사</button>
        ...
        <div className={isMobile ? "" : "dashboard-main-grid"} style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '320px 1fr', gap: isMobile ? '12px' : '30px' }}>
          <aside style={{ display: isMobile && !showPalette ? 'none' : 'flex', flexDirection: 'column', gap: '20px' }}>
             ...
             {!isMobile && (
               <div className="glass" style={{ borderRadius: 'var(--radius-lg)', background: 'white', padding: '20px' }}>
                 <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '15px' }}><AppWindow size={20} color="var(--primary)"/><h3 style={{ fontSize: '16px', fontWeight: '700' }}>학습 어플 바로가기</h3></div>
                 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
                   {studyApps.map(app => (
                     <button key={app.id} onClick={() => window.open(app.url, '_blank')} className="glass" style={{ padding: '12px 8px', borderRadius: '12px', border: 'none', cursor: 'pointer', textAlign: 'center', fontSize: '12px', fontWeight: '800', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px' }}>
                       <div style={{ width: '36px', height: '36px', background: 'var(--bg-gradient)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{app.name[0]}</div>
                       {app.name}
                     </button>
                   ))}
                 </div>
          ...
          <main><TimeGrid tasks={currentTasks} onUpdateTask={updateTask} onDeleteTask={deleteTask} isAdmin={isAdmin} /></main>
        </div>
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
            <div className="glass animate-fade-in" style={{ background: 'white', padding: '25px', borderRadius: '24px', maxWidth: '400px', width: '100%' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h2 style={{ fontSize: '18px', fontWeight: '900', color: 'var(--primary)' }}>학습 어플 모음</h2>
                <button onClick={() => setShowAppLauncher(false)} style={{ background: 'none', border: 'none' }}><CloseIcon size={20}/></button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '15px', marginBottom: '20px' }}>
                {studyApps.map(app => (
                  <div key={app.id} style={{ position: 'relative' }}>
                    <button onClick={() => window.open(app.url, '_blank')} style={{ width: '100%', aspectRatio: '1/1', background: 'rgba(0,0,0,0.03)', border: 'none', borderRadius: '18px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '5px', cursor: 'pointer' }}>
                      <div style={{ fontSize: '20px', fontWeight: '900', color: 'var(--primary)' }}>{app.name[0]}</div>
                      <div style={{ fontSize: '11px', fontWeight: '700' }}>{app.name}</div>
                    </button>
                ...
                {isAdmin && <button onClick={() => { const name = prompt('어플 이름'); const url = prompt('주소(URL)'); if(name && url){ setAppName(name); setAppUrl(url); } }} style={{ width: '100%', aspectRatio: '1/1', border: '2px dashed #ddd', borderRadius: '18px', background: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Plus size={24} color="#aaa"/></button>}
              </div>
              {isAdmin && appName && appUrl && <button onClick={addStudyApp} className="btn-primary" style={{ width: '100%' }}>등록하기</button>}
            </div>
          </div>
        )}
        ...
        {showMessageManager && isAdmin && (
           <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
             <div className="glass animate-fade-in" style={{ background: 'white', padding: '25px', borderRadius: '24px', maxWidth: '400px', width: '100%' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}><h2 style={{ fontSize: '18px', fontWeight: '900', color: '#ff4d6d' }}>깜짝 메시지 보내기</h2><button onClick={() => setShowMessageManager(false)} style={{ background: 'none', border: 'none' }}><CloseIcon size={20}/></button></div>
                ...
                <textarea className="input-field" style={{ height: '100px', marginBottom: '15px' }} placeholder="메시지를 입력하세요" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} />
                <button onClick={sendSurpriseMessage} className="btn-primary" style={{ background: '#ff4d6d', border: 'none' }}>보내기</button>
             </div>
           </div>
        )}
        ...
        {showSurprise && todayMessage && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
            <div className="glass animate-bounce-in" style={{ background: 'white', padding: '30px', borderRadius: '24px', maxWidth: '400px', width: '100%', textAlign: 'center' }}>
              ...
              <h2 style={{ fontSize: '20px', fontWeight: '900', marginBottom: '15px' }}>엄마가 보내는 깜짝 편지!</h2>
              ...
              <button onClick={() => setShowSurprise(false)} className="btn-primary" style={{ background: '#ff4d6d', border: 'none' }}>확인했어요!</button>
            </div>
          </div>
        )}
           ...
           <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
             <div className="glass animate-fade-in" style={{ background: 'white', padding: '25px', borderRadius: '24px', maxWidth: '400px', width: '100%' }}>
               ...
               <div style={{ display: 'grid', gap: '15px' }}>
                 ...
                 <button className="btn-primary" disabled={passwordBusy} onClick={submitPasswordChange}>비밀번호 변경</button>
               </div>
             </div>
           </div>
        )}
        ...
        {showGoals && (
           <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
             <div className="glass animate-fade-in" style={{ background: 'white', padding: '25px', borderRadius: '24px', maxWidth: '400px', width: '100%' }}>
               ...
               <div style={{ display: 'flex', gap: '5px', marginBottom: '15px' }}><input className="input-field" placeholder="목표를 입력하세요" value={newGoal} onChange={(e) => setNewGoal(e.target.value)} /><button onClick={() => { if(newGoal){ applyGoalsChange(prev => [...prev, {id:Date.now(), text:newGoal, done:false}]); setNewGoal(''); } }} className="btn-primary" style={{ width: '50px' }}>+</button></div>
               <div style={{ maxHeight: '300px', overflowY: 'auto' }}>
                 {goals.map(g => <div key={g.id} onClick={() => applyGoalsChange(prev => prev.map(i => i.id === g.id ? {...i, done: !i.done} : i))} style={{ padding: '12px', background: g.done ? '#f0fff4' : '#f9f9f9', marginBottom: '8px', borderRadius: '12px', fontSize: '14px', cursor: 'pointer', textDecoration: g.done ? 'line-through' : 'none', opacity: g.done ? 0.6 : 1 }}>{g.text}</div>)}
               </div>
             </div>
           </div>
        )}
