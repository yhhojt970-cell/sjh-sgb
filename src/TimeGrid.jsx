  ...
  return (
    ...
    >
      ...
      <div style={{ 
        ...
        {hourTasks.map(t => (
          <div key={t.id} style={{ flex: isMobile ? '1 1 100%' : '1 1 calc(33.333% - 10px)', minWidth: isMobile ? '100%' : '250px' }}>
            <TaskCard task={t} onUpdateTask={onUpdateTask} onDeleteTask={onDeleteTask} isAdmin={isAdmin} isMobile={isMobile} />
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div ref={setNodeRef} {...attributes} {...listeners} style={style}>
      ...
      <div style={{ display: 'flex', gap: '6px', marginTop: '10px' }}>
        {task.type === 'class' ? (
          <>
            <button onPointerDown={(e) => { e.stopPropagation(); onUpdateTask(task.id, { completed: !task.completed, status: 'completed', coins: 1 }) }} style={{ flex: 1, padding: '8px', borderRadius: '10px', background: task.status === 'completed' ? '#42c99b' : '#f1f5f9', color: task.status === 'completed' ? 'white' : '#666', border: 'none', fontWeight: 'bold', cursor: 'pointer', fontSize: isMobile ? '12px' : '13px' }}>완료</button>
            <button onPointerDown={(e) => { e.stopPropagation(); onUpdateTask(task.id, { completed: false, status: 'holiday' }) }} style={{ flex: 1, padding: '8px', borderRadius: '10px', background: task.status === 'holiday' ? '#3b82f6' : '#f1f5f9', color: task.status === 'holiday' ? 'white' : '#666', border: 'none', fontWeight: 'bold', cursor: 'pointer', fontSize: isMobile ? '12px' : '13px' }}>휴강</button>
            <button onPointerDown={(e) => { e.stopPropagation(); onUpdateTask(task.id, { completed: false, status: 'absent' }) }} style={{ flex: 1, padding: '8px', borderRadius: '10px', background: task.status === 'absent' ? '#ef4444' : '#f1f5f9', color: task.status === 'absent' ? 'white' : '#666', border: 'none', fontWeight: 'bold', cursor: 'pointer', fontSize: isMobile ? '12px' : '13px' }}>결석</button>
          </>
        ) : (
          <>
            ...
            {task.startTimeActual && !task.completed && <button onPointerDown={(e) => { e.stopPropagation(); handleComplete() }} style={{ flex: 1, padding: '8px', borderRadius: '10px', background: '#42c99b', color: 'white', border: 'none', fontWeight: 'bold', cursor: 'pointer', fontSize: isMobile ? '12px' : '13px' }}>완료</button>}
            {task.completed && <div style={{ fontSize: '11px', color: '#42c99b', fontWeight: 'bold', lineHeight: 1.2 }}>✨ {task.startTimeActual} ~ {task.endTimeActual} ({task.durationActual}분 공부 완료!)</div>}
          </>
        )}
      </div>
    </div>
