...
// Fixed Colors for the ORIGINAL style
...
function Dashboard({ user = {}, onLogout, allUsers = {}, cloud = {} }) {
  ...
  const [showSettings, setShowSettings] = useState(false); const [showGoals, setShowGoals] = useState(false); const [showMessageManager, setShowMessageManager] = useState(false); const [showAppLauncher, setShowAppLauncher] = useState(false); const [showSurprise, setShowSurprise] = useState(false)
  
  const [newGoal, setNewGoal] = useState(''); const [newReward, setNewReward] = useState({ text: '', coins: 50 }); const [newMessage, setNewMessage] = useState(''); const [messageTarget, setMessageTarget] = useState('')
  ...
  useEffect(() => {
    if (!activeKidId && Object.keys(allUsers).length > 0) {
      ...
      if (kids.length > 0) {
          const sjh = kids.find(k => k.includes('지희')) || kids[0]
          setActiveKidId(sjh)
      }
    }
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={e => { const d = e.active.data.current; if (d?.type === 'palette') setActiveDragItem({ type: 'palette', subject: d.subject }); else if (d?.type === 'task') setActiveDragItem({ type: 'task', task: d.task }); }} onDragEnd={e => { const { active, over } = e; if (!over) { setActiveDragItem(null); return }; const d = active.data.current; if (d?.type === 'palette' && over.id.toString().startsWith('hour-')) { const startTime = `${over.data.current.hour.toString().padStart(2, '0')}:00`; const nt = { id: Math.random().toString(36).substr(2, 9), name: d.subject.name, color: d.subject.color, startTime, expectedEndTime: '00:00', duration: 50, type: 'study', icon: 'Book', completed: false, date: todayStr, coins: 1 }; const next = [...tasks, nt]; setTasks(next); persist({ tasks: next }) } setActiveDragItem(null); }}>
      <div className="dashboard-shell" style={{ maxWidth: '1200px', margin: '0 auto', padding: isMobile ? '10px' : '20px', minHeight: '100vh', background: '#fff9fb' }}>
        
        {/* ORIGINAL STYLE HEADER */}
        <header className="glass" style={{ padding: '15px 25px', borderRadius: '20px', marginBottom: '15px', background: 'white', border: `1px solid ${BORDER_COLOR}`, boxShadow: '0 4px 12px rgba(255, 77, 109, 0.05)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '6px', marginTop: '5px' }}>
                  {kidsList.map(id => (
                    <button key={id} onClick={() => setActiveKidId(id)} style={{ padding: '4px 12px', borderRadius: '8px', border: activeKidId === id ? `2px solid ${PRIMARY_PINK}` : '1px solid #ddd', background: activeKidId === id ? LIGHT_PINK : 'white', fontSize: '11px', fontWeight: 'bold', color: activeKidId === id ? PRIMARY_PINK : '#666' }}>{allUsers[id]?.displayName || id}</button>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {isAdmin && <button onClick={() => setShowMessageManager(true)} className="header-btn-original"><Mail size={20}/></button>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '300px 1fr', gap: '20px' }}>
          ...
          <main><TimeGrid tasks={todayTasks} onUpdateTask={(id, up) => { const next = tasks.map(t => t.id === id ? {...t, ...up} : t); setTasks(next); persist({ tasks: next }) }} onDeleteTask={id => { const next = tasks.filter(t => t.id !== id); setTasks(next); persist({ tasks: next }) }} isAdmin={isAdmin} essentialChecklist={essentials} onAddSpecialEvent={() => {}} /></main>
        </div>

        {/* RESTORED ORIGINAL MODALS */}
        {showGoals && (
           <div className="modal-overlay" onClick={() => setShowGoals(false)}>
             <div className="modal-content glass animate-fade-in" onClick={e => e.stopPropagation()} style={{background:'white', borderRadius:'24px', padding:'30px', maxWidth:'400px', width:'100%'}}>
               <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}><h2 style={{fontWeight:'900', color:PRIMARY_PINK}}>목표 및 관리 🏆</h2><button onClick={() => setShowGoals(false)} style={{border:'none', background:'none'}}><CloseIcon size={24}/></button></div>
               
               <div style={{background:LIGHT_PINK, padding:'20px', borderRadius:'18px', textAlign:'center', marginBottom:'20px'}}>현재 코인: <strong style={{fontSize:'32px'}}>{availableCoins}</strong></div>
               
               {isAdmin && (
                 <div style={{ marginBottom: '20px', padding: '15px', background: 'rgba(255, 77, 109, 0.05)', borderRadius: '18px' }}>
                   <h3 style={{ fontSize: '14px', fontWeight: '900', color: PRIMARY_PINK, marginBottom: '10px' }}>보상 추가하기</h3>
                   <div style={{ display: 'flex', gap: '5px' }}>
                     ...
                     <button onClick={() => { if(newReward.text){ const next=[...rewards, {id:Date.now(), ...newReward}]; setRewards(next); persist({rewards:next}); setNewReward({text:'', coins:50}); } }} className="btn-primary" style={{width:'50px'}}><Plus/></button>
                   </div>
                 </div>
               )}
        ...
        {showSurprise && todayMessage && (
           <div className="modal-overlay" onClick={() => { setShowSurprise(false); updateDoc(doc(cloud.db, 'households', cloud.householdId, 'meta', 'messages'), { messages: messages.map(m => m.id === todayMessage.id ? {...m, read: true} : m) }) }}>
             <div className="modal-content glass animate-bounce-in" onClick={e => e.stopPropagation()} style={{textAlign:'center', background:'white', borderRadius:'30px', padding:'40px', maxWidth:'350px'}}>
               ...
               <div style={{background:'#fff9fb', padding:'25px', borderRadius:'18px', marginBottom:'30px', fontWeight:'700', border:`1px dashed ${PRIMARY_PINK}`, color:'#555', fontSize:'17px'}}>{todayMessage.text}</div>
               <button onClick={() => { setShowSurprise(false); updateDoc(doc(cloud.db, 'households', cloud.householdId, 'meta', 'messages'), { messages: messages.map(m => m.id === todayMessage.id ? {...m, read: true} : m) }) }} className="btn-primary" style={{width:'100%', padding:'18px', fontSize:'18px'}}>확인 ❤️</button>
             </div>
           </div>
        )}
        ...
        {showMessageManager && isAdmin && (
           <div className="modal-overlay" onClick={() => setShowMessageManager(false)}>
             <div className="modal-content glass animate-fade-in" onClick={e => e.stopPropagation()} style={{background:'white', borderRadius:'24px', padding:'30px', maxWidth:'400px', width:'100%'}}>
               <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}><h2 style={{fontWeight:'900', color:PRIMARY_PINK}}>깜짝 메시지 보내기</h2><button onClick={() => setShowMessageManager(false)} style={{border:'none', background:'none'}}><CloseIcon/></button></div>
               <div style={{display:'grid', gap:'10px'}}>
                 ...
                 <textarea className="input-field" style={{height:'120px'}} placeholder="메시지 입력" value={newMessage} onChange={e => setNewMessage(e.target.value)} />
                 <button onClick={() => { if(newMessage && messageTarget){ updateDoc(doc(cloud.db, 'households', cloud.householdId, 'meta', 'messages'), { messages: arrayUnion({ id: Date.now(), text: newMessage, date: todayStr, kidId: messageTarget, read: false }) }); setNewMessage(''); setShowMessageManager(false); } }} className="btn-primary">전송하기</button>
               </div>
             </div>
           </div>
        )}
      ...
      <DragOverlay>{activeDragItem && <div style={{ padding: '12px', background: 'white', borderRadius: '12px', borderLeft: `6px solid ${PRIMARY_PINK}`, boxShadow: '0 8px 24px rgba(0,0,0,0.15)', fontWeight: '900' }}>{activeDragItem.type === 'palette' ? activeDragItem.subject.name : activeDragItem.task.name}</div>}</DragOverlay>
    </DndContext>
  )
}
